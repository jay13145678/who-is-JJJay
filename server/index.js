...
// 静态文件：serve 项目根目录（index.html, css/, js/, assets/）
...
app.post('/api/chat', async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({
      error: '未配置 API Key。请复制 server/.env.example 为 server/.env 并填入密钥。',
    });
  }

  const model = process.env.LLM_MODEL || 'deepseek-chat';

  // 构建消息列表：系统提示 + 历史消息 + 当前问题
  const messages = [
    SYSTEM_PROMPT,
    ...history.slice(-10), // 最多保留最近 10 轮历史
